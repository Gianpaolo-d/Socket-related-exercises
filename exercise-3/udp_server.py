"""
UDP Server — Ping Pong with count and unreliable channel simulation (Exercise 3)
...
import random   # used to simulate unreliable channel behavior (message drops)
...
DROP_PROBABILITY = 0.3  # probability of simulating a dropped message (30% chance)
...
def sendReply(sock, reply, client_addr):
    ...
    To simulate an unreliable channel, this function randomly decides to
    drop the message (i.e., not send it) based on a predefined probability.
    If the message is "dropped", it simply logs this event without sending
    the reply. 
    ...
    if random.random() < DROP_PROBABILITY:
        print(f"[Server] Simulating dropped message: {reply!r} to {client_addr}\n")
        return  # Simulate a dropped message by not sending the reply
